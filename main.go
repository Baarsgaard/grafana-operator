...
func main() { // nolint:gocyclo
	...
	if maxConcurrentReconcilesStr != "" {
		max, err := strconv.Atoi(maxConcurrentReconcilesStr)
		if err != nil {
			setupLog.Error(
				fmt.Errorf("invalid value %s for %s: %w", maxConcurrentReconcilesStr, maxConcurrentReconcilesEnvVar, err),
				fmt.Sprintf("falling back to a max concurrency of %s", maxConcurrentReconciles),
			)
		} else {
